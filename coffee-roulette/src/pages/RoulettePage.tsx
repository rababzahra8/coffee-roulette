...
export function RoulettePage() {
  ...
  const { selectedIngredients, setCurrentRecipe, setIsSpinning } = useApp()
  const [phase, setPhase] = useState<'spin' | 'land' | 'done'>('spin')
  const [flyingChips, setFlyingChips] = useState<string[]>([])

  useEffect(() => {
    setIsSpinning(true)
    setFlyingChips(selectedIngredients.length > 0 ? selectedIngredients : ['Espresso', 'Milk', 'Sugar'])
  ...
  }, [navigate, selectedIngredients, setCurrentRecipe, setIsSpinning])
